...
if __name__ == '__main__':
    ...
    # 指定数据集文件夹的路径。
    #dataset_path = '/home/user/Desktop/isaac_project/debug_output_F_efficientnet'
    '''dataset path includes path to all folder with only 000001-000022 (without hard negatives)'''
    dataset_path = '/home/user/Desktop/isaac_project/debug_output_C'
    ...
    # 是否启用 EfficientNet。
    efficientnet_key = None#"efficientnet_b4"
    ...
    test_bop_dataset_back_front_item = test_bop_dataset_back_front(bop_dataset_item, train_folder_name, padding_ratio=padding_ratio, ratio=1)#ratio=0.01)
        
    HELD_OUT_SCENE = '000022'

    # Filter train: exclude the held-out scene
    for obj_key in train_bop_dataset_back_front_item.dataset_info['obj_info']:
        train_bop_dataset_back_front_item.dataset_info['obj_info'][obj_key] = [
            s for s in train_bop_dataset_back_front_item.dataset_info['obj_info'][obj_key]
            if s['scene'] != HELD_OUT_SCENE
        ]

        #print("obk info", train_bop_dataset_back_front_item.dataset_info['obj_info'], '\n')

    # Filter test: keep only the held-out scene
    for obj_key in test_bop_dataset_back_front_item.dataset_info['obj_info']:
        test_bop_dataset_back_front_item.dataset_info['obj_info'][obj_key] = [
            s for s in test_bop_dataset_back_front_item.dataset_info['obj_info'][obj_key]
            if s['scene'] == HELD_OUT_SCENE
        ]
    ...
    for obj_id in range(start_obj_id, end_obj_id + 1):
        ...
        test_loader = torch.utils.data.DataLoader(test_bop_dataset_back_front_item, batch_size=4, 
